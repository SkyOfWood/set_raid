...
x86_64() {
    if [[ -f raid_ctrl_x86_64.tar.gz ]]; then
        if [[ $(md5sum raid_ctrl_x86_64.tar.gz | awk '{print$1}') != $(curl -sk $URL/md5sum_x86_64.txt) ]]; then
            curl -skO $URL/raid_ctrl_x86_64.tar.gz && tar zxf raid_ctrl_x86_64.tar.gz
        else
            tar zxf raid_ctrl_x86_64.tar.gz
        fi
    else
        curl -skO $URL/raid_ctrl_x86_64.tar.gz && tar zxf raid_ctrl_x86_64.tar.gz
    fi
...
aarch64() {
    if [[ -f raid_ctrl_aarch64.tar.gz ]]; then
        if [[ $(md5sum raid_ctrl_aarch64.tar.gz | awk '{print$1}') != $(curl -sk $URL/md5sum_aarch64.txt) ]]; then
            curl -skO $URL/raid_ctrl_aarch64.tar.gz && tar zxf raid_ctrl_aarch64.tar.gz
        else
            tar zxf raid_ctrl_aarch64.tar.gz
            rpm -ivh *.rpm --force --nodeps
            ln -s /opt/MegaRAID/storcli/storcli64 .
            ln -s /usr/bin/jq .
        fi
    else
        curl -skO $URL/raid_ctrl_aarch64.tar.gz
        tar zxf raid_ctrl_aarch64.tar.gz
        rpm -ivh *.rpm --force --nodeps
        ln -s /opt/MegaRAID/storcli/storcli64 .
        ln -s /usr/bin/jq .
...
VD_Num=$(expr $(
    ABC=$(./jq -r ".\"Controllers\"[].\"Response Data\".\"Virtual Drives\"" raid_all.conf.json)
    echo $ABC | sed 's/ / + /g' | xargs expr
) - 1)
...
get_disk_type() {
    ...
    PD_Num=$(expr $(
        ABC=$(./jq -r ".\"Controllers\"[].\"Response Data\".\"Physical Drives\"" raid_all.conf.json)
        echo $ABC | sed 's/ / + /g' | xargs expr
    ) - 1)
...
help_raid() {
    ...
    echo_INFO "Author : wuchenglin@kylinos.cn"
